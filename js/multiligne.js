...
function multiLigne(data) {
  ...
  cty.xScale = d3.scaleLinear().domain([2000, 2022]).range([50, 1550]);

  cty.yScale = d3.scaleLinear().domain([0, 100]).range([950, 20]);
  ...
  data.forEach((element) => {
    if (element.country != current) {
      ...
      cty.arrayData.push([
        {
          pays: current,
          sdg: element.sdg_index_score,
          goal1: element.goal_1_score,
